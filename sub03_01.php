...
$beauty = $_GET["beauty"];
$result = mysqli_query($con,"Select * From article where kind ='5'&& small_kind='$beauty' group by product_name order by time desc") or die("Error");
session_start();
?>
<!DOCTYPE html>
<html lang="en">
...
<body style="background-image: url('img/back.png')!important;">
  ...
  <div class="" style="margin-top: 13%; margin-bottom: 3%;">
    <h2 class="text-center active" onclick="productType('all')">美食</h2>
      <ul class="nav justify-content-center" id="myTab" role="tablist">
    <?php $small=array("中式","西式","日式","小吃","飲品");
  for($i=0;$i<count($small);$i++){
  ?>
  
  
        <li class="nav-item" role="presentation">
  <a href="sub03_01.php?beauty=<?php echo $i;?>"class="nav-link"><h5><?php echo $small[$i];?></h5></a>
        </li>
      <?php }?>
      </ul>
  </div>
            ...
            <div class="card" style="width: 12rem;">
             <?php
              ...
              <h5 class="card-title text-center"><a href="product.php?pid=<?php echo $row['product_name']?>"><?php echo $row['product_name']?></a></h5>
                <?php 
                if($row['mark']=='廠商貼文'){?>
                <?php echo '<img src="img/crown.png" width="30px" height="30px">';
              }else{}?></h5></a>
              </div>
            </div>
        </div>
