...
$beauty = $_GET["beauty"];
$result = mysqli_query($con,"Select * From article where kind ='3'&& small_kind='$beauty' group by product_name order by time desc") or die("Error");
session_start();
?>
...
<body style="background-image: url('img/back.png')!important;">
  <div class="header" style="position: fixed; top:0;">
   ...
   <?php if(isset($_SESSION['people'])){?>
    ...
    <?php if(isset($_SESSION['people']['member_id'])){?></div><?php
      ...
      <?php
    echo "親愛的",$_SESSION['people']['company_name'],"歡迎進入";?></div><?php
  ...
  <div class="" style="margin-top: 13%; margin-bottom: 3%;">
    <h2 class="text-center active" onclick="productType('all')">3c</h2>
      <ul class="nav justify-content-center" id="myTab" role="tablist">
    <?php $small=array("筆電","桌電","手機","攝材");
  for($i=0;$i<count($small);$i++){
  ?>
  
  
        <li class="nav-item" role="presentation">
  <a href="sub01_01.php?beauty=<?php echo $i;?>"class="nav-link"><h5><?php echo $small[$i];?></h5></a>
        </li>
      <?php }?>
      </ul>
  </div>
